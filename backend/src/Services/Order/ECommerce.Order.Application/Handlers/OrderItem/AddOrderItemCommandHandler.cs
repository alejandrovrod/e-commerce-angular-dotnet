...
public class AddOrderItemCommandHandler : IRequestHandler<AddOrderItemCommand, ApiResponse<OrderDto>>
{
    ...
    public async Task<ApiResponse<OrderDto>> Handle(AddOrderItemCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Add item to order using the existing method
            order.AddItem(request.ProductId, request.ProductName, request.UnitPrice, request.Quantity, request.ProductSku);
        ...
        {
            return ApiResponse<OrderDto>.ErrorResult($"Error adding item to order: {ex.Message}");
        }
    }
