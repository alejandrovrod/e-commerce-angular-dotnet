...
public class RemoveOrderItemCommandHandler : IRequestHandler<RemoveOrderItemCommand, ApiResponse<OrderDto>>
{
    ...
    public async Task<ApiResponse<OrderDto>> Handle(RemoveOrderItemCommand request, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            return ApiResponse<OrderDto>.ErrorResult($"Error removing item from order: {ex.Message}");
        }
    }
