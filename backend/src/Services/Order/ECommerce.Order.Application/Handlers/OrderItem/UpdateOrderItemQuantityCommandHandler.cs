...
namespace ECommerce.Order.Application.Handlers.OrderItem;

public class UpdateOrderItemQuantityCommandHandler : IRequestHandler<UpdateOrderItemQuantityCommand, ApiResponse<OrderDto>>
{
    ...
    public async Task<ApiResponse<OrderDto>> Handle(UpdateOrderItemQuantityCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Update item quantity using the existing method
        ...
        {
            return ApiResponse<OrderDto>.ErrorResult($"Error updating item quantity: {ex.Message}");
        }
    }
