...
public class GetOrdersByUserQueryHandler : IRequestHandler<GetOrdersByUserQuery, ApiResponse<List<OrderDto>>>
{
    private readonly IOrderRepository _orderRepository;
    private readonly IUnitOfWork _unitOfWork;

    public GetOrdersByUserQueryHandler(IOrderRepository orderRepository, IUnitOfWork unitOfWork)
    {
        _orderRepository = orderRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<ApiResponse<List<OrderDto>>> Handle(GetOrdersByUserQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return ApiResponse<List<OrderDto>>.SuccessResult(new List<OrderDto>());
            }
        ...
        {
            return ApiResponse<List<OrderDto>>.ErrorResult($"Error retrieving orders for user: {ex.Message}");
        }
    }
