...
public class GetPaymentsByUserQueryHandler : IRequestHandler<GetPaymentsByUserQuery, ApiResponse<List<PaymentDto>>>
{
    ...
    public async Task<ApiResponse<List<PaymentDto>>> Handle(GetPaymentsByUserQuery request, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            return ApiResponse<List<PaymentDto>>.ErrorResult($"Error retrieving payments for user: {ex.Message}");
        }
    }
